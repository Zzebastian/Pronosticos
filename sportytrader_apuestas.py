...
def version1x2(url, Elementos1x2, accion=1, filtro = 70):
  ...
  if accion in (1,5):
    if os.path.exists('Pronosticos/pronosticos1x2.csv'):
      conf = True
    else:
      conf = False

    with open('Pronosticos/pronosticos1x2.csv', 'a', newline='', encoding='utf-8') as f:
...
def versionProbabilidad(urlCuota, ElementoCuota, base, impresion=True):
  ...
  for partido in encuentros:
    dato = partido['Equipo1']+' - '+partido['Equipo2']
    dato2 = partido['Equipo2']+' - '+partido['Equipo1']
    for diccionario in direcciones:
      if diccionario['Equipo'] == dato or diccionario['Equipo'] == dato2:
        partido['link'] = diccionario['Link']

  if impresion == True:  
    col = 30
    for partido in encuentros:
      print(f'{partido["Equipo1"]:<{col}} | {partido["Equipo2"]:<{col}} | {partido["Ganador"]:<{3}} | {partido["Probabilidad"]:<{5}}| {partido["Fecha"]}| {partido["link"]}')
